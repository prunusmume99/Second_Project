...
class RoutineManager:
    ...
    def single_touch(self, card):
        ...
        # 공부시간 조회
        period = 'today' if self.mode == 1 else 'month'
        data = query_study_time(card, period=period)
        # LCD 메시지 구성
        if self.mode == 1:
            message = f"오늘 공부시간: {data}"
        else:
            message = f"이번달 공부시간: {data}"
        print(f"📺 LCD -> {message}")
        return {'mode': self.mode, 'lcd': message}
...
def query_study_time(card, period='today'):
    ...
    if period == 'today':
        sql = """
        SELECT SEC_TO_TIME(SUM(TIMESTAMPDIFF(SECOND, Start_Time, End_Time)))
        FROM Record_Table
        WHERE Card_Num=%s
